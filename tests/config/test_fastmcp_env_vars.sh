...
echo "Building Docker image for FastMCP environment variable test..."
docker build -t mcp-bigquery-server-fastmcp:latest .

echo "Running FastMCP container to test environment variables in response..."
docker run -i --rm \
  ...
  -e LOCATION=US \
  mcp-bigquery-server-fastmcp:latest \
  --stdio << EOF
