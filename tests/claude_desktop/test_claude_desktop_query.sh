...
echo "Building Docker image for Claude Desktop query test..."
docker build -t mcp-bigquery-server-test:latest .

echo "Running container to test Claude Desktop query pattern..."
docker run -i --rm \
...
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"execute_query","arguments":{"sql":"SELECT * FROM \`amazon-study-db\`.INFORMATION_SCHEMA.SCHEMATA LIMIT 5","projectId":"amazon-study-db","location":"US"}},"id":3}
EOF
