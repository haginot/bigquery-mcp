...
echo "Building Docker image for final Claude Desktop test..."
docker build -t mcp-bigquery-server-final:latest .

echo "Running container to simulate Claude Desktop configuration..."
docker run -i --rm \
  ...
  -e LOCATION=US \
  mcp-bigquery-server-final:latest \
  --stdio << EOF
